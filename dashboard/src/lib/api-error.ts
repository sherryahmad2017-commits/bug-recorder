...
// since the dashboard and API are separate deployables (see docs/ARCHITECTURE.md §5).
...
export async function parseErrorResponse(res: Response): Promise<ApiError> {
  ...
  return new ApiError(res.status, err?.code ?? 'internal_error', err?.message ?? 'Something went wrong. Please try again.', err?.details);
}
